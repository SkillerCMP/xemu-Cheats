...
EXPECTED_CURRENT_GAME_METHOD_COUNT = 24
EXPECTED_CURRENT_GAME_METHOD_DIGEST = (
    "485697ca52d8c73afbee324d479618bf98cf939a14a97f542a86f3e4b8bc6b02"
)
...
def protected_current_game_digest(text: str) -> tuple[int, str]:
    functions = sorted(
        (item for item in extract_member_functions(text, "CurrentGameManager")
         if item[0] not in {"Refresh", "Draw", "DrawGameInfoTab"}),
...
def main() -> int:
    ...
    refresh = extract_function(cc, "void CurrentGameManager::Refresh(bool force)")
    for needle in (
    ...
    if "RefreshDisc(false);" not in unchanged or "return;" not in unchanged:
        raise AssertionError("unchanged XBE fast path stopped polling mounted-disc identity")
